...
int __devinit msm_thermal_init(struct msm_thermal_data *pdata)
{
	int ret = 0;
	uint32_t cpu;
//[All][Main][CPU][36812] Fixed Camera can not boot up in low temperature by QCT Patch 20140423 S
	for_each_possible_cpu(cpu) {
		cpus[cpu].cpu = cpu;
		cpus[cpu].offline = 0;
...
//[All][Main][CPU][36812] Fixed Camera can not boot up in low temperature by QCT Patch 20140423 E	
	BUG_ON(!pdata);
